...
const TestTable = () => {
  return (
    <Formik initialValues={{ data: [{ key: 1, name: "lorem ipsum" }] }} onSubmit={() => {}}>
      <Form>
        <AddRowButton name="data" data-testid="add" createNewRow={()=>({key:2,name:"new item"})}/>
          <Table name="data"
          pagination={false}
            columns={[{
              title: "Name",
              key: "name",
              dataIndex: "name",
              render: (text,record,index)=><Input name={`data.${index}.name`}/>
            },
            {
              title: "Actions",
              key: "actions",
              render: (text,record,index)=><RemoveRowButton data-testid="remove" name="data" index={index}>remove</RemoveRowButton>
            }
            ]}
          />
      </Form>
